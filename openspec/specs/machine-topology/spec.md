...
The guest container SHALL run the chart's shim image. A machine's rootfs source SHALL NOT be used
as any container's image.

The machine's operating system lives in the persistent volume, seeded once from the source. The
container image only provides the small program that mounts that volume and hands control to the
guest's init. Conflating the two would make an OCI source look like a normal container image and
