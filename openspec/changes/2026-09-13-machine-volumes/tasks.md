...
- [x] 1.1 Add `helm unittest` cases for every refusal in the `values-validation` delta, and verify
  each fails against the unmodified chart for the right reason
- [x] 1.2 Add the resolution helper to `_helpers.tpl`: for one machine, emit the declared volumes
...
- [x] 1.3 Implement the validation, accumulating into the existing semantic stage; verify `make
  test` is green and every message names the machine and the volume

## 2. The objects

- [x] 2.1 Add `helm unittest` cases asserting that a sized volume renders a claim template with its
...
- [x] 2.2 Render the claim templates, the pod volumes and the guest's mounts in `statefulset.yaml`
- [x] 2.3 Verify `make test` and `make conform` are green at both the development version and the
  chart's floor

## 3. Documentation

- [x] 3.1 Document the block in `values.yaml` at the point of use - the permanence of the name, the
  immutability of the size, the paths the boot sequence owns, and what mounting over populated
  content does; run `make docs`
- [x] 3.2 Add an example under `charts/stateful-pods/examples/`, and confirm `make lint` and
  `make conform` cover it
- [x] 3.3 Extend `NOTES.txt` to report each declared volume, the claim behind it and the path it is
  mounted at
- [x] 3.4 Update the chart README and the project README

## 4. On a cluster

- [x] 4.1 Extend `hack/integration-test.sh`: a machine with a declared volume boots, the path inside
