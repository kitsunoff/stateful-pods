...
- [ ] 1.1 Add `helm unittest` cases for every refusal in the `values-validation` delta — a network
...
- [ ] 1.2 Add the resolution helper to `_helpers.tpl`: for one machine, emit the declared ports in
  a normalised form with the protocol defaulted, deterministically ordered
- [ ] 1.3 Implement the validation the cases from 1.1 demand, accumulating into the existing
  semantic stage; verify `make test` is green and every message names the machine and the input

## 2. The objects

- [ ] 2.1 Add `helm unittest` cases asserting that declared ports appear on the guest container and
...
- [ ] 2.2 Render the ports on the guest container in `statefulset.yaml` and on the Service in
  `service.yaml`
- [ ] 2.3 Add `helm unittest` cases for the policy: rendered only when asked, named for the
...
- [ ] 2.4 Add `templates/network-policy.yaml`; verify `make test` and `make conform` are green at
  both the development version and the chart's floor

## 3. Documentation

- [ ] 3.1 Document the block in `values.yaml` at the point of use, including what a container port
  list is and is not, and what a NetworkPolicy depends on; run `make docs`
- [ ] 3.2 Add an example under `charts/stateful-pods/examples/` declaring ports and asking for them
  to be enforced, and confirm `make lint` and `make conform` cover it
- [ ] 3.3 Extend `NOTES.txt` to report the declared ports, the DNS name each is reachable at, and
  the warning when the posture admits nothing
- [ ] 3.4 Update the chart README and the project README

## 4. On a cluster

- [ ] 4.1 Extend `hack/integration-test.sh`: a machine declaring ports renders them on the pod and
